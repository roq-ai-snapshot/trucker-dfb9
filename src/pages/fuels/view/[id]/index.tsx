...
import { getFuelById } from 'apiSdk/fuels';
import { FuelInterface } from 'interfaces/fuel';

function FuelViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<FuelInterface>(
    () => (id ? `/fuels/${id}` : null),
    () =>
      getFuelById(id, {
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('fuel', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/fuels/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                <FormListItem label="Fuel Type" text={data?.fuel_type} />
