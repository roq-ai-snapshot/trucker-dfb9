...
type ColumnType = ColumnDef<ServiceInterface, unknown>;
...
export function ServiceListPage(props: ServiceListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getServices({
        ...
        order: params.order,
        searchTermKeys: ['service_info.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<ServiceInterface>>(
    () => `/services?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: ServiceInterface) => {
  ...
  const columns: ColumnType[] = [
    { id: 'service_info', header: 'Service Info', accessorKey: 'service_info' },
