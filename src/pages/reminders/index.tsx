...
export function ReminderListPage(props: ReminderListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getReminders({
        ...
        order: params.order,
        searchTermKeys: ['reminder_info.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<ReminderInterface>>(
    () => `/reminders?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const columns: ColumnType[] = [
    { id: 'reminder_info', header: 'Reminder Info', accessorKey: 'reminder_info' },
