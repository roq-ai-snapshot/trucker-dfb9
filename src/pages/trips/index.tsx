...
import { getTrips, deleteTripById } from 'apiSdk/trips';
import { TripInterface } from 'interfaces/trip';

type ColumnType = ColumnDef<TripInterface, unknown>;
...
export function TripListPage(props: TripListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<TripInterface>>(
    () => `/trips?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: TripInterface) => {
    if (hasAccess('trip', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/trips/view/${row.id}`);
    }
  };

  const columns: ColumnType[] = [
    { id: 'trip_cost', header: 'trip_cost', accessorKey: 'trip_cost' },
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/trips/view/${record.id}`} passHref legacyBehavior>
            <Button
          ...
          {hasAccess('trip', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/trips/edit/${record.id}`} passHref legacyBehavior>
              <Button
                onClick={(e) => e.stopPropagation()}
                mr={2}
                padding="0rem 0.5rem"
          ...
          {hasAccess('trip', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
            <IconButton
              onClick={(e) => {
                e.stopPropagation();
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="trip" />
        </Flex>

        {hasAccess('trip', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/trips/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
